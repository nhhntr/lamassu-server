...
function plugins (settings, deviceId) {
  function buildRates (tickers) {
    ...
    cryptoCodes.forEach((cryptoCode, i) => {
      ...
      const cashInCommission = BN(1).plus(BN(cryptoConfig.cashInCommission).div(100))

      const cashOutCommission = _.isNil(cryptoConfig.cashOutCommission)
        ? undefined
        : BN(1).plus(BN(cryptoConfig.cashOutCommission).div(100))
  ...
  function fiatBalance (fiatCode, cryptoCode) {
    ...
    return Promise.all([
      ...
      .then(([rates, balanceRec]) => {
        ...
        return {timestamp: balanceRec.timestamp, balance: fiatTransferBalance.toFixed(0)}
      })
  }
  ...
  */
  ...
  function buyAndSell (rec, doBuy) {
    ...
    const cryptoAtoms = doBuy ? rec.cryptoAtoms : rec.cryptoAtoms.negated()
  ...
  function randomCode () {
    return BN(crypto.randomBytes(3).toString('hex'), 16).shiftedBy(-6).toFixed(6).slice(-6)
  }
