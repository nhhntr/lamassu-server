...
function computeCrypto (cryptoCode, _balance) {
  ...
  return BN(_balance).shiftedBy(-unitScale).round(5)
}
...
function computeFiat (rate, cryptoCode, _balance) {
  ...
  return BN(_balance).shiftedBy(-unitScale).times(rate).round(5)
}
    ...
    .then(([settings, machineList]) => {
      ...
      return Promise.all(promises)
        .then(([fundingRec, ratesRec]) => {
          ...
          const pending = fundingRec.fundingPendingBalance.minus(fundingConfirmedBalance)
